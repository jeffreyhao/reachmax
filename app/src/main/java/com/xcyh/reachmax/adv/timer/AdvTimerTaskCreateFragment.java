...
import com.xcyh.reachmax.databinding.FragmentCreateTimerBinding;
 ...
 * 创建定时开关任务
 */
public class AdvTimerTaskCreateFragment extends BaseDialogFragment<FragmentCreateTimerBinding, EmptyPresenter> {



    public static AdvTimerTaskCreateFragment newInstance(int level, ItemData data, SubmitListener listener) {
        AdvTimerTaskCreateFragment chapterSheetDialogFragment = new AdvTimerTaskCreateFragment();
    ...
    private boolean mActionOpen = true;
    ...
    public void initListener() {
        mBinding.tvStartThrow.setOnClickListener(v->{
            mActionOpen = true;
            mBinding.tvStartThrow.setChecked(true);
            mBinding.tvCloseThrow.setChecked(false);
        });
        mBinding.tvCloseThrow.setOnClickListener(v->{
            mActionOpen = false;
            mBinding.tvStartThrow.setChecked(false);
            mBinding.tvCloseThrow.setChecked(true);
        });
    ...
    protected void initData() {
        ...
        if(mItemData == null){
            ToastUtils.showToastCenter("itemData 不能为 null");
            return;
        }

        mTask = mItemData.getSwitchTask();
        mActionOpen = initAction(mTask);
        ...
        if(mTask == null){
            ...
            mBinding.tvStartThrow.setChecked(mActionOpen);
            mBinding.tvCloseThrow.setChecked(!mActionOpen);
        }

        mBinding.tvTimerSetting.setText(mSelectStartTime);
        mBinding.tvTaskName.setText(mItemData.getName(mLevel));
    }

    private void setActionState(){
        String state = mItemData.getState(mLevel);
        switch (state){
            case AdvActionState.ACTIVE:
                mActionOpen = false;

                mBinding.tvStartThrow.setEnabled(false);
                mBinding.tvCloseThrow.setEnabled(true);
            ...
            case AdvActionState.PAUSED:
                ...
                mBinding.tvStartThrow.setEnabled(true);
                mBinding.tvCloseThrow.setEnabled(false);
    ...
    private boolean initAction(TaskBean task){
        if(task == null){
            return true;
        }
        ...
        return action.equals(AdvActionState.ACTIVE);
    }
    ...
    private void doSubmit(@ClickAction int action, @TaskStatus int status){
        ...
        if(mSubmitListener != null){
            ...
            bundle.putString("action", mActionOpen ? AdvActionState.ACTIVE : AdvActionState.PAUSED);
