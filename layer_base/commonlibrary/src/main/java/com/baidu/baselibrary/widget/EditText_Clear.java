...
public class EditText_Clear extends androidx.appcompat.widget.AppCompatEditText {
     ...
     * 步骤1：定义左侧搜索图标 & 一键删除图标
     */
    private Drawable clearDrawable,leftDrawable;
     ...
     */
    private void init() {
        clearDrawable = getResources().getDrawable(R.drawable.ic_search_delete_input);
        leftDrawable = getResources().getDrawable(R.drawable.ic_search_grey);
        setCompoundDrawablesRelativeWithIntrinsicBounds(leftDrawable, null,  null, null);
        boolean isRTLMode = AppUtil.isRTLMode();
        if(isRTLMode) {
...
//         setCompoundDrawablesWithIntrinsicBounds(Drawable left, Drawable top, Drawable right, Drawable bottom)介绍
    ...
    private void setClearIconVisible(boolean visible) {
        setCompoundDrawablesRelativeWithIntrinsicBounds(leftDrawable, null, visible ? clearDrawable : null, null);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                boolean isRTLMode = AppUtil.isRTLMode();
                if ((!isRTLMode && drawable != null && event.getX() <= (getWidth() - getPaddingRight())
                        && event.getX() >= (getWidth() - getPaddingRight() - drawable.getBounds().width()))||
                        (isRTLMode && event.getX() >= getPaddingLeft()
